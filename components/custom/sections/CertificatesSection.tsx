        ...
        <div className="overflow-hidden" ref={emblaRef}>
          <div className="flex touch-pan-y -ml-6">
            {certificates.map((cert, index) => (
              ...
              >
                <div className={`h-full p-8 rounded-[30px] border border-border bg-gradient-to-br ${cert.color} backdrop-blur-sm flex flex-col items-start gap-6`}>
                  <div className={`p-4 rounded-2xl bg-background border border-border shadow-sm ${cert.iconColor}`}>
                    <Award className="w-8 h-8" />
                  </div>
                  <div>
                    <h3 className="text-2xl font-bold mb-2">{cert.title}</h3>
                    <p className="text-muted-foreground font-medium">{cert.issuer}</p>
                    <p className="text-sm font-bold text-primary mt-4">{cert.date}</p>
                  </div>
                </div>
              </div>
            ))}
